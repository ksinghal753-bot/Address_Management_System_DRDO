...
def build_test_elements(record, styles):
    ...
    sp_style = styles['sp']
    to_style = styles['to_label']
    line_style = styles['address_line']
    
    # TOP RIGHT: NO/Date
    header_table = Table(
        [
            ["", Paragraph(f"{no_line}<br/>{date_line}", ref_style)]
        ],
    ...
    envelope_data.append(["", "", Paragraph("Director", line_style)])
    envelope_data.append(["", "", Paragraph("DRDO", line_style)])
    envelope_data.append(["", "", Paragraph("Agra, UP - 282001", line_style)])
    
    address_table = Table(
        envelope_data,
        colWidths=[10*cm, 1.5*cm, 14*cm]
    )
    ...
    elems.append(address_table)
    
    # Spacer to push to bottom? 
    # Actually we can just use a large spacer, or calculate it.
    elems.append(Spacer(1, 4*cm))
    
    # BOTTOM: Sender Box (Left) and By S/P (Right)
...
Post Box No. 51, Station Road, Agra Cantt-282001""", sender_style)
    ...
    bottom_table = Table(
        [
            [sender_table, Paragraph(f"<b>By S/P:</b> {sp_fmt}", sp_style)]
        ],
        colWidths=[15*cm, 10.7*cm]
    )
...
def test():
    doc = SimpleDocTemplate('scratch/test_env2.pdf', pagesize=landscape(A4), topMargin=2*cm, bottomMargin=1.5*cm, leftMargin=2*cm, rightMargin=2*cm)
    rec = {
