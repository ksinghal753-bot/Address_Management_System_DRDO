...
def build_test_elements(record, styles):
    ...
    bold_style = styles['address_bold']
    
    # TOP HEADER: NO/Date on left, By S/P on right
    header_table = Table(
        [
            [Paragraph(f"{no_line}<br/>{date_line}", ref_style), Paragraph(f"<b>By S/P:</b> {sp_fmt}", sp_style)]
        ],
...
def test():
    doc = SimpleDocTemplate('scratch/test_env3.pdf', pagesize=landscape(A4), topMargin=2*cm, bottomMargin=1.5*cm, leftMargin=2*cm, rightMargin=2*cm)
    rec = {
