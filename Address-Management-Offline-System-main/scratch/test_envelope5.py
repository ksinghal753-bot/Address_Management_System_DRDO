...
def build_test_elements(record, styles):
    ...
    for _ in range(5):
        envelope_data.append(["", "", Paragraph("Director Address Line testing testing", line_style)])
    
    # INCREASED LEFT OFFSET
    address_table = Table(
        envelope_data,
        colWidths=[11.5*cm, 1.5*cm, 12.7*cm]
    )
    ...
    # INCREASED BOTTOM SPACER TO COMPENSATE FOR TOP SPACER REDUCTION
    elems.append(Spacer(1, 3.0*cm))
...
def test():
    doc = SimpleDocTemplate('scratch/test_env5.pdf', pagesize=landscape(A4), topMargin=2*cm, bottomMargin=1.5*cm, leftMargin=2*cm, rightMargin=2*cm)
    rec = {
